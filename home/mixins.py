...
class ContributionSettings(BaseSetting):
    ...
    callout_snippet = models.ForeignKey(ContributionCallout, null=True, on_delete=models.SET_NULL, verbose_name="Efni til að birta neðst í greinum")

    panels = [
        SnippetChooserPanel('callout_snippet'),
        FieldPanel('callout_active')
    ]
...
class GeneralSiteSettings(BaseSetting):
    base_template = models.ForeignKey(BaseTemplate, null=True, on_delete=models.SET_NULL)
...
class SiteSettingsTemplateResponseMixin(object):
    ...
    def get_template_names(self):
        ...
        app_settings = GeneralSiteSettings.for_site(request.site)
        template = super(SiteSettingsTemplateResponseMixin, self).get_template_names()[0]
