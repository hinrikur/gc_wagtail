...
def parse_csv_as_table(table_data, table_caption, table_header_row,
                       table_header_column, table_bootstrap, table_footnote, separator='|'):
...
@register.simple_tag()
def parse_csv_as_kox_table(table_data, table_caption, table_header_row,
                       table_header_column, table_bootstrap, table_footnote, separator='|'):
        ...
        for cell_num, cell in enumerate(cells):
            ...
            if row_num == 0 and table_header_row:
                ...
                if cell_num == 0 and table_header_column:
                    ...
                    html += '<td><div class="color" style="background-color:rgba(140, 198, 63, %s)">' % cell
                    html += '<div class="row-hover">%.0f%s</div></div></td>' % ((float(cell) * 100), '%')
...
def latest_articles(category_slug, order='-first_published_at', count=5, offset=0):
    pages = ArticlePage.objects.filter(category__slug=category_slug).order_by(order)[offset:count+offset]
    return {
