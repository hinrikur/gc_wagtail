...
class SvarBlock(blocks.StructBlock):
    ...
    svar = blocks.ChoiceBlock(choices=(('ja', 'Ja'),('nei', 'Nei'), ('svarar_ekki', 'Svarar ekki'), ('ekki_akvedid', 'Ekki akvedid')), required=False)
...
class SocialBlock(blocks.StructBlock):
    facebook = blocks.BooleanBlock(required=False, help_text=_('Add Facebook share option'))
    twitter = blocks.BooleanBlock(required=False, help_text=_('Add Twitter share option'))
    twitter_text = blocks.CharBlock(required=False, max_length=140, help_text=_('Custom share text for Twitter. Max 140 spaces.'))
...
class TableBlock(blocks.StructBlock):
    ...
    ], help_text="Veldu breidd töflunnar. Allar breiddir verða full-width í minnstu tækjum (col-xs-12).", required=True)
    table = blocks.TextBlock(rows=10, help_text=_('Enter your table as comma separated values, one line for each row.'))
    caption = blocks.CharBlock(required=False)
    footnote = blocks.TextBlock(required=False, help_text=_('Enter notes to explain your data'))
    header_row = blocks.BooleanBlock(required=False, help_text=_('Render first row as header if checked'))
    header_column = blocks.BooleanBlock(required=False, help_text=_('Render first column as header if checked'))
    bootstrap = blocks.CharBlock(required=False, default='table ', help_text=_('Styles: table-striped, table-bordered, table-hover, table-condensed'))
    block_classes = blocks.CharBlock(required=False)
...
class NoRepTableBlock(blocks.StructBlock):
    ...
    table = blocks.TextBlock(rows=10, help_text=_('Only numeric data. Enter your table as comma separated values, one line for each row.'))
    caption = blocks.CharBlock(required=False)
    header_row = blocks.BooleanBlock(required=False, help_text=_('Render first row as header if checked'))
    header_column = blocks.BooleanBlock(required=False, help_text=_('Render first column as header if checked'))
    bootstrap = blocks.CharBlock(required=False, help_text=_('Styles: table-striped, table-bordered, table-hover, table-condensed'))
    footnote = blocks.TextBlock(required=False, help_text=_('Enter notes to explain your data'))
    block_classes = blocks.CharBlock(required=False, help_text=_('text-center, text-right, border-left, white'))
    break_container = blocks.BooleanBlock(required=False, default=False, help_text=_('Notist með Container Block'))
...
class PageChooserBlock(blocks.StructBlock):
    title = blocks.CharBlock(required=False, help_text=_('Optional title with featured content'))
...
class TagListBlock(blocks.StructBlock):
    ...
    tag = blocks.CharBlock(help_text="Ath að hástafir og lágstafir skipta máli í töggum.")
    count = blocks.IntegerBlock(default=10, help_text="Fjöldi atriða í lista sem birtist. Tíu atriði eru sjálfgefin.")
    block_classes = blocks.CharBlock(required=False)
...
class BasicHeaderBlock(blocks.StructBlock):
    ...
    ], help_text="Veldu stærð myndarinnar.", required=True)
    extra_image = ImageChooserBlock(required=False, help_text="Veldu sérstaka mynd til að birta yfir grein. Slepptu til að nota Header Image.")
    crop_image = blocks.BooleanBlock(required=False, default=False, help_text="Viltu nota fókus svæði myndar? Þetta skilyrðir hlutföll mynda.")
    bgvideo_link = blocks.URLBlock(required=False, label="Bakgrunnsmyndband slóð", help_text="Bakgrunnsmynd verður myndband frá YouTube eða Vimeo")
    bgvideo_mp4 = DocumentChooserBlock(required=False, label="Bakgrunnsmyndband hýst", help_text="Bakgrunnsmynd verður myndband sem hýst er hjá Kjarnanum.")
    extra_title = blocks.CharBlock(required=False, classname="full title", help_text="Sérstök fyrirsögn yfir grein. Slepptu til að birta Title.")
    extra_subtitle = blocks.CharBlock(required=False, classname="subtitle", help_text="Sérstök undirfyrirsögn yfir grein. Slepptu til að birta Subtitle.")
    calluna = blocks.BooleanBlock(required=False, help_text="Viltu hafa letrið Calluna í fyrirsögn?")
    display_category = blocks.BooleanBlock(required=False, default=True, help_text="Viltu birta category-miða?")
    display_author = blocks.BooleanBlock(required=False, default=True, help_text="Viltu birta nafn höfundar og mynd?")
    display_social = blocks.BooleanBlock(required=False, default=True, help_text="Viltu birta tengla á samfélagsmiðla?")
    block_classes = blocks.CharBlock(required=False)
...
class ArticlePageBlock(blocks.StreamBlock):
    paragraph = RichTextBlock(template="kjarninn/articles/block_templates/paragraph_block.html")
