...
class Author(models.Model, index.Indexed):
    ...
    list_cluster = models.IntegerField(
        verbose_name=_("Ordering"),
        default=99,
        help_text="Vinsamlegast notið tölur 0-99",
    )
    panels = [
    ...
    def get_children(self, count=50):
        ...
        ids += (
            ...
            .filter(Q(author_name_override="") | Q(author_name_override=None))
            .order_by("-first_published_at")
            .values_list("pk", flat=True)[:count]
        )
        ids += (
            self.blockarticlepage_set.filter(live=True)
            .filter(Q(author_name_override="") | Q(author_name_override=None))
            .order_by("-first_published_at")
            .values_list("pk", flat=True)[:count]
        )
        ids += (
            self.mediapage_set.filter(live=True)
            .filter(Q(author_name_override="") | Q(author_name_override=None))
            .order_by("-first_published_at")
            .values_list("pk", flat=True)[:count]
        )
        return (
            Page.objects.filter(pk__in=ids)
            .order_by("-first_published_at")
            .specific()[:count]
        )
...
class TagIndexPage(Page):
    def get_context(self, request):
        ...
        articles = articles.order_by(
            "-first_published_at", "-go_live_at", "latest_revision_created_at"
        )
...
class ArticleTag(TaggedItemBase):
    content_object = ParentalKey(
        ...
        null=True,
        blank=True,
        related_name="%(app_label)s_%(class)s_taggeditems",
    )
...
class Category(models.Model, index.Indexed):
    ...
    backend_title = models.CharField(
        max_length=255, null=True, blank=False, verbose_name=_("Bakendatitill")
    )
...
class MovePagesMixin(object):
    ...
    def move(self, target, pos=None):
        ...
        logger.info(
            "Redirect to old url path created: %s => %s", r.old_path, r.redirect_page
        )


class AuthorMixin(models.Model):
    author = models.ForeignKey(
        Author, null=True, blank=False, on_delete=models.SET_NULL
    )
...
class ArticlePage(
    SiteSettingsTemplateMixin,
    MovePagesMixin,
    Page,
    RelatedPageMixin,
    CategoryMixin,
    ArticleTagStuffMixin,
    AuthorMixin,
    ArticleMixin,
):
    ...
    editors_notes = models.TextField(
        blank=True,
        null=True,
        help_text="<br> býr til nýja línu. <i>...</i> skáletrar. <b>...</b> feitletrar.",
    )
    note_date = models.DateField(
        max_length=255,
        blank=True,
        null=True,
        help_text="Dagsetning athugasemdar. Valkvætt.",
    )
    related_links_title = models.CharField(
        max_length=80, blank=True, null=True, help_text="Shortcode: [links]"
    )
    sub_article_title = models.CharField(max_length=255, blank=True, null=True)
    sub_article = RichTextField(
        blank=True, help_text="Shortcode: [box] — <p> skilgreinir meginmál."
    )
    sub_article_layout = models.CharField(
        max_length=255, null=True, choices=SUB_ARTICLE_LAYOUT_CHOICES, default="none"
    )
    header_image_crop = models.BooleanField(
        default=False, help_text="Viltu láta myndskurð ráðast af fókus svæði?"
    )
    header_image_layout = models.CharField(
        max_length=225,
        choices=HEADER_IMAGE_LAYOUT_CHOICES,
        default="default",
        help_text="Veldu myndastillingu á aðalmynd. Hefur aðeins áhrif á þessari síðu.",
    )
    header_video = models.CharField(
        max_length=225,
        blank=True,
        null=True,
        help_text="Slóð á myndbönd frá YouTube og Vimeo.",
    )
    ...
    aukahlutir_panels = [
        ...
        MultiFieldPanel(
            ...
            heading="Athugasemd ritstjórnar",
            classname="collapsible",
        ),
    ]
